...
#define CURR_DIR_BUFF_SIZE PATH_MAX
...
int main(int argc , char* argv[]){
    ...
    char currDirBuffer[CURR_DIR_BUFF_SIZE];
    char inputBuffer[INPUT_BUFF_SIZE];
    cmd_line* cmdLine;

    while(1){
        getcwd(currDirBuffer,CURR_DIR_BUFF_SIZE);
        printf("%s:",currDirBuffer);
        if(fgets(inputBuffer, INPUT_BUFF_SIZE, stdin)!=NULL){
