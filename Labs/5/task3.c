...
int execute(cmd_line *line) {
    ...
    int i = 0;
    if (line->output_redirect && line->input_redirect) {
        ...
        if (!outputFile) {
            outputFile = fopen(line->output_redirect, "w+");
        }
        if (!inputFile) {
            perror("open failed");
            free_cmd_lines(line);
        ...
        i = execvp(line->arguments[0], line->arguments);
        if (i == -1) {
            perror("Error");
            free_cmd_lines(line);
            _exit(0);
        ...
        return i;
    } else if (!line->output_redirect && line->input_redirect) {
...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (fgets(inputBuffer, 2048, stdin) != NULL) {
            ...
            if (cmdLine->arguments == 0) {
                free_cmd_lines(cmdLine);
            } else if (cmdLine->blocking) {
