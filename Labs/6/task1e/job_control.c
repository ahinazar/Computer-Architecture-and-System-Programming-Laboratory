...
void run_job_in_foreground(job **job_list, job *j, int cont, struct termios *shell_tmodes, pid_t shell_pgid) {
    int status;
    if (waitpid(j->pgid, &status, WNOHANG) ==
        -1) { //Check if the job is done. If it fails (returns -1) then there are no processes with the given process group id.
        j->status = DONE; //it prints a Done message in the same format as in print_jobs and remove the job from the job list.
        printf("[%d]\t %s \t\t %s", j->idx, status_to_str(j->status), j->cmd);
        remove_job(job_list, j);
    } else { // the job has not finished yet
        tcsetpgrp(STDIN_FILENO, j->pgid); //put it in the foreground
        ...
        while (waitpid(j->pgid, &status, WUNTRACED) != -1) { // Wait for the job to change status
            if (WIFSTOPPED(status)) {
                if (WSTOPSIG(status) == SIGTSTP) { //(ctrl-z)
                    j->status = SUSPENDED;
                    break;
                } else if (WSTOPSIG(status) == SIGINT) { //(ctrl-c)
                    j->status = DONE;
                    break;
                }
            }
        ...
        tcsetattr(STDIN_FILENO, TCSADRAIN, shell_tmodes); //Restore the shell’s terminal attributes
    }
    update_job_list(job_list, 1); //Check for status update of jobs that are running in the background
}
...
void run_job_in_background(job *j, int cont) {
    j->status = RUNNING; //sets its status to RUNNING
    if (cont)
        kill(j->pgid, SIGCONT); //ends it a SIGCONT
}