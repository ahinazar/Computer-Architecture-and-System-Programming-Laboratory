...
void inputRedirect(cmd_line *line) {
    close(STDIN_FILENO);
...
void outputRedirect(cmd_line *line) {
    close(STDOUT_FILENO);
...
void closeAndDuplicateRight(int rightPipe[]) {
    close(STDOUT_FILENO);
    dup(rightPipe[1]);
}

void closeAndDuplicateLeft(int leftPipe[]) {
    close(STDIN_FILENO);
    dup(leftPipe[0]);
}

int execute(cmd_line *line, int leftPipe[], int rightPipe[], job *job1, job **job_list, pid_t pid,
    ...
    if (childPid == 0) {
        ...
        if (rightPipe) {
            closeAndDuplicateRight(rightPipe);
        }

        if (leftPipe) {
            closeAndDuplicateLeft(leftPipe);
        }
...
int main(int argc, char *argv[]) {
    ...
    tcgetattr(STDIN_FILENO,
              termios1); //Save default terminal attributes to restore them back when a process running in the foreground ends.
    ...
    while (1) {
        ...
        if (fgets(inputBuffer, inputBufferSize, stdin) != NULL) {
